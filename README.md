...
<p align="center">
  <a href="https://github.com/anthropics/claude-code"><img src="https://img.shields.io/badge/based%20on-Claude%20Code%20v2.1.88-blueviolet" alt="Based on Claude Code v2.1.88"></a>
...
> 首次在 HarmonyOS PC 上使用？请参阅下方 [HarmonyOS PC 详细指南](#harmonyos-pc-详细指南)，了解从零开始的完整步骤。
...
# 或直接安装 .tgz：
npm install -g npm-dist/claude-code-ohos-2.1.88-ohos.1.tgz
```

npm `postinstall` 脚本会自动：
- 下载对应平台的 ripgrep
- 将 `start-claude.sh` 安装到 `~/.claude/start-claude.sh`（启动脚本模板）
- 将 `.env.example` 安装到 `~/.claude/.env.example`（环境变量模板）

安装完成后，配置 API 凭据：

```bash
cp ~/.claude/.env.example ~/.claude/.env
vi ~/.claude/.env   # 填入实际的 API Key 和端点
```
...
在 [Releases](https://github.com/chenjh16/HarmonyOS-Claude-Code/releases) 页面下载 `claude-code-ohos-<version>.tgz`，无需从源码构建。下载后直接传输到 HarmonyOS PC 安装即可。
...
在 HiShell 中创建工作目录并下载：

```bash
mkdir -p ~/Claude && cd ~/Claude
curl -s http://127.0.0.1:19090/claude-code-ohos-2.1.88-ohos.1.tgz -o ~/Claude/claude-code.tgz
```
...
Claude Code 需要连接 LLM API 端点。`postinstall` 已自动安装环境变量模板到 `~/.claude/.env.example`。复制并编辑：
...
`start-claude.sh` 会在启动时自动加载 `~/.claude/.env`。环境变量示例：
...
然后在 `~/Claude` 目录中运行（避免在 home 目录直接启动）：
...
└── postinstall-rg.mjs npm 安装后脚本（ripgrep + start-claude.sh + .env.example）

start-claude.sh          HarmonyOS PC 一键启动脚本（安装到 ~/.claude/）
.env.example             环境变量模板（安装到 ~/.claude/）
```
...
> First time on HarmonyOS PC? See the [HarmonyOS PC Guide](#harmonyos-pc-guide) below for a complete step-by-step walkthrough.
...
- Installs `.env.example` to `~/.claude/.env.example` (environment variable template)

Configure API credentials:

```bash
cp ~/.claude/.env.example ~/.claude/.env
vi ~/.claude/.env   # fill in your actual values
```

Set `CLAUDE_CODE_SKIP_RG_INSTALL=1` to skip ripgrep download.

After installation, configure API credentials via `cp ~/.claude/.env.example ~/.claude/.env && vi ~/.claude/.env`, then run from `~/Claude` directory:
...
Download `claude-code-ohos-<version>.tgz` from the [Releases](https://github.com/chenjh16/HarmonyOS-Claude-Code/releases) page — no need to build from source. Transfer it to HarmonyOS PC and install directly.
...
Then in HiShell on HarmonyOS PC, create the working directory and download:
...
Claude Code needs an API endpoint to connect to an LLM. The `postinstall` script automatically installed an environment variable template to `~/.claude/.env.example`. Copy and configure it:

```bash
cp ~/.claude/.env.example ~/.claude/.env
vi ~/.claude/.env   # fill in your actual API Key and endpoint
```

`start-claude.sh` automatically loads `~/.claude/.env` at startup.
...
Then run from the `~/Claude` directory (avoid launching from home directory directly):
...
└── postinstall-rg.mjs npm postinstall script (ripgrep + start-claude.sh + .env.example)

start-claude.sh          HarmonyOS PC one-click startup script (installed to ~/.claude/)
.env.example             Environment variable template (installed to ~/.claude/)
```
