...
import { ensureEmbeddedRipgrep } from './rg-embedded.js'
...
// Bun's define replaces MACRO.VERSION at bundle time. Cache at module scope
// so it survives async context issues (see bun#26168).
declare const MACRO: { VERSION: string }
const APP_VERSION = MACRO.VERSION

const isOpenHarmony = process.platform === ('openharmony' as NodeJS.Platform)

function getOpenHarmonyTmpDir(): string {
  const base = process.env.CLAUDE_CODE_TMPDIR || tmpdir()
  const dir = path.join(base, 'rg-io')
  try {
...
const getRipgrepConfig = memoize((): RipgrepConfig => {
  ...
  // 1. Try extracted embedded ripgrep first (standalone builds).
  //    The binary is embedded via Bun.embeddedFiles and lazily extracted
  //    to ~/.claude/bin/rg on first run or version change.
  if (isInBundledMode() && !userWantsSystemRipgrep) {
    try {
      const extractedPath = ensureEmbeddedRipgrep(APP_VERSION)
      if (extractedPath) {
        return { mode: 'embedded', command: extractedPath, args: [] }
      }
    } catch {
  ...
  // 2. Try system-installed ripgrep on PATH.
  const { cmd: systemPath } = findExecutable('rg', [])
  if (systemPath !== 'rg') {
    return { mode: 'system', command: 'rg', args: [] }
  }

  // 2b. Try postinstall-downloaded ripgrep at ~/.claude/bin/rg.
  //     The npm postinstall script downloads rg here (and on HarmonyOS PC,
  //     signs it with binary-sign-tool).
  const postinstallRg = path.join(
    homedir(),
    '.claude',
    'bin',
  ...
  try {
    accessSync(postinstallRg, constants.X_OK)
    return { mode: 'system', command: postinstallRg, args: [] }
  } catch {
    // not found or not executable — fall through to vendored
  }

  // 3. Try vendored ripgrep (npm installs with bundled rg).
...
/**
 * On OpenHarmony, Node.js pipe-based stdout capture from child processes is
 * broken — rg exits with code 0 but produces zero bytes on the pipe. File I/O
 * and PTY I/O work fine. This workaround redirects rg stdout/stderr to temp
 * files and reads them back after the process exits.
 */
function ripGrepRawViaFile(
  rgPath: string,
  fullArgs: string[],
  ...
  const tmpDir = getOpenHarmonyTmpDir()
  const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  const stdoutFile = path.join(tmpDir, `rg-${id}.out`)
  const stderrFile = path.join(tmpDir, `rg-${id}.err`)

  const escapedArgs = fullArgs.map(shellEscapeArg).join(' ')
  const cmd = `${shellEscapeArg(rgPath)} ${escapedArgs} > ${shellEscapeArg(stdoutFile)} 2> ${shellEscapeArg(stderrFile)}`
  ...
  child.on('close', (code, signal) => {
    ...
    try {
      ...
      /* file may not exist on early termination */
    }
    try {
  ...
  child.on('error', (err: NodeJS.ErrnoException) => {
    ...
    clearTimeout(killTimeoutId)
    cleanup()
    callback(err as ExecFileException, '', '')
  })
...
function ripGrepRaw(
  ...
  if (isOpenHarmony) {
    return ripGrepRawViaFile(rgPath, fullArgs, abortSignal, callback)
  }
...
async function ripGrepFileCount(
  ...
  if (isOpenHarmony) {
    return new Promise<number>((resolve, reject) => {
      const tmpDir = getOpenHarmonyTmpDir()
      const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
      const stdoutFile = path.join(tmpDir, `rg-count-${id}.out`)
      const escapedArgs = fullArgs.map(shellEscapeArg).join(' ')
      const cmd = `${shellEscapeArg(rgPath)} ${escapedArgs} > ${shellEscapeArg(stdoutFile)} 2>/dev/null`
      ...
      child.on('close', code => {
        ...
        try {
          ...
          lines = countCharInString(content, '\n')
        } catch {
          /* file may not exist */
        }
        try {
...
export async function ripGrepStream(
  ...
  // On OpenHarmony, pipe capture is broken — fall back to file-based I/O.
  // Streaming is lost but correctness is preserved.
  if (isOpenHarmony) {
    return new Promise<void>((resolve, reject) => {
      const tmpDir = getOpenHarmonyTmpDir()
      const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
      const stdoutFile = path.join(tmpDir, `rg-stream-${id}.out`)
      const escapedArgs = fullArgs.map(shellEscapeArg).join(' ')
      const cmd = `${shellEscapeArg(rgPath)} ${escapedArgs} > ${shellEscapeArg(stdoutFile)} 2>/dev/null`
...
const testRipgrepOnFirstUse = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    if (config.argv0) {
      ...
      // On OpenHarmony, pipe capture is broken; use file-based workaround
      const tmpDir = getOpenHarmonyTmpDir()
      const versionFile = path.join(tmpDir, `rg-version-${Date.now()}.out`)
      const escapedCmd = `${shellEscapeArg(config.command)} ${config.args.map(shellEscapeArg).join(' ')} --version > ${shellEscapeArg(versionFile)} 2>/dev/null`
      const { code } = await execFileNoThrow('/bin/sh', ['-c', escapedCmd], {
        timeout: 5000,
      })
      let stdout = ''
      try {
        stdout = readFileSync(versionFile, 'utf8')
      } catch {
        /* file may not exist */
      }
      try {
