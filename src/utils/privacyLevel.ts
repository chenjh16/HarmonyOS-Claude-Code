...
/**
 * True when Anthropic account-related network access should be suppressed.
 * Covers: OAuth, settings sync, transcript sharing, metrics opt-out, referral,
 * overage credit grants, Claude AI limits, Grove notifications, bootstrap API.
 *
 * Parallel to isEssentialTrafficOnly() — either can independently disable
 * account features.
 */
export function isAnthropicAccountDisabled(): boolean {
  return !!process.env.CLAUDE_CODE_SKIP_ANTHROPIC_ACCOUNT
}
